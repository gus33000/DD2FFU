...
namespace Decomp.Microsoft.WindowsPhone.Imaging
{
    public class FileIdentifier : BaseIdentifier, IBlockIoIdentifier, IDeviceIdentifier
    {
        ...
        public BcdElementBootDevice ParentDevice
        {
            get; set;
        }
        ...
        public void ReadFromStream(BinaryReader reader)
        {
            ...
            if (reader.BaseStream.Position - position >= Length)
            {
                throw new ImageStorageException(string.Format(
                                ...
                                MethodBase.GetCurrentMethod().Name, position, Version, Length,
                                Type));
            }
        ...
        public void LogInfo(IULogger logger, int indentLevel)
        {
            ...
            logger.LogInfo(str + "Version:       {0}", Version);
            logger.LogInfo(str + "Length:        {0}", Length);
            logger.LogInfo(str + "Type:          {0}", Type);
            logger.LogInfo(str + "Path:          {0}", Path);
            logger.LogInfo("");
