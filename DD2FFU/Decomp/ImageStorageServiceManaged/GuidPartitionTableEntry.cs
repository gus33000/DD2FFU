...
namespace Decomp.Microsoft.WindowsPhone.Imaging
{
    internal class GuidPartitionTableEntry
    {
        ...
        public void WriteToStream(Stream stream, int bytesPerEntry)
        {
            ...
            var byteArray1 = PartitionType.ToByteArray();
            binaryWriter.Write(byteArray1, 0, byteArray1.Length);
            var byteArray2 = PartitionId.ToByteArray();
            ...
            if (stream.Position - position >= bytesPerEntry)
                return;
            var num = (int) (bytesPerEntry - (stream.Position - position));
        ...
        public void LogInfo(ushort indentLevel = 0)
        {
            ...
            _logger.LogInfo(str + "Partition Name : {0}", (object) PartitionName);
            _logger.LogInfo(str + "Partition Type : {{{0}}}", (object) PartitionType);
            _logger.LogInfo(str + "Partition Id   : {{{0}}}", (object) PartitionId);
            _logger.LogInfo(str + "Starting Sector: 0x{0:x}", (object) StartingSector);
            _logger.LogInfo(str + "Last Sector    : 0x{0:x}", (object) LastSector);
            _logger.LogInfo(str + "Attributes     : 0x{0:x}", (object) Attributes);
            _logger.LogInfo("");
        }
