...
namespace Decomp.Microsoft.WindowsPhone.Imaging
{
    public class PartitionIdentifierEx : BaseIdentifier, IDeviceIdentifier
    {
        ...
        public void LogInfo(IULogger logger, int indentLevel)
        {
            ...
            if (ParentIdentifier != null)
            {
                if (ParentIdentifier.BlockType == BlockIoType.HardDisk ||
                    ParentIdentifier.BlockType == BlockIoType.VirtualHardDisk)
                             ...
                             (ParentIdentifier as VirtualDiskIdentifier).InternalIdentifer).PartitionStyle)
                    {
                        ...
                        case PartitionFormat.Mbr:
                            logger.LogInfo(str + "MBR Partition Offset: 0x{0:x}", (object) MbrPartitionOffset);
                            break;
        ...
        public uint Size
        {
            ...
            {
                if (ParentIdentifier != null)
                    return 16U + Math.Max(ParentIdentifier.Size, BlockIoIdentifierFactory.SizeOnDisk);
